            ...
            @if($recipes && sizeof($recipes) > 0)
                @foreach($recipes as $r)
                    <div class="uk-width-1-1@xs uk-width-1-1@m uk-child-width-1-1 blog-box " uk-grid>
                        ...
                        <div class="uk-width-1-1@xs uk-width-2-5@s uk-height-medium uk-background-cover uk-border-rounded" data-src="{{asset($r->image_url)}}"  uk-img>

                        </div>
                        <div class="uk-width-expand">
                            <h2>{{$r->name}}</h2>
                            <div class="text-small">
                            <span class="uk-badge">Tezina: {{$r->difficulty}}</span>
                                <span uk-icon="icon: calendar"></span>
                            <span>{{\Carbon\Carbon::parse($r->created_at)->format('d.m.Y')}}</span>
                                <span uk-icon="icon: comment"></span>
                                <span>{{count($r->comments)}}</span>

                            </div>
                            <p class="text-normal">{!!Str::limit($r->text, 250)!!}</p>
                            <a class="uk-button uk-button-text" href="/recept/{{$r->id}}">Read more</a>
